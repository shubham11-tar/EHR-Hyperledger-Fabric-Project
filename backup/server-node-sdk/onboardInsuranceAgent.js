...
async function main() {
    try {
        ...
        const ccpPath = path.resolve(__dirname, 'cert-script', 'connection-org2.json');
        ...
        const userIdentity = await wallet.get('insuranceAgent-Rama01');
        if (userIdentity) {
            console.log('An identity for the user "insuranceAgent-Rama01" already exists in the wallet');
            return;
        }
        ...
        const secret = await ca.register({
            ...
            enrollmentID: 'insuranceAgent-Rama01',
            role: 'client',
            attrs: [{ name: 'role', value: 'agent', ecert: true },{ name: 'uuid', value: 'insuranceAgent-Rama01', ecert: true }],
        }, adminUser);
        ...
        await wallet.put('insuranceAgent-Rama01', x509Identity);
        console.log('Successfully registered and enrolled insuranceAdmin user "insuranceAgent-Rama01" and imported it into the wallet');
        ...
        console.error(`Failed to register user "insuranceAgent-Rama01": ${error}`);
        process.exit(1);
      }
}
